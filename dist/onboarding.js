...
async function runOnboarding() {
    ...
    if (mode === 'reset') {
        const { runResetWizard } = await Promise.resolve().then(() => __importStar(require('./tui/resetWizard')));
        await runResetWizard();
        return;
    }
    const step1 = await inquirer_1.default.prompt([
        {
            type: 'list',
            name: 'provider',
    ...
    const step2 = await inquirer_1.default.prompt([
        {
            type: 'list',
            name: 'modelSelection',
    ...
    if (model === 'Custom...') {
        const step2Custom = await inquirer_1.default.prompt([
            {
                type: 'input',
                name: 'customModel',
    ...
    const defaultWebhookSecret = crypto_1.default.randomBytes(16).toString('hex');
...
`;
    ...
    if (answers.telegramToken.trim())
        envContent += `TELEGRAM_BOT_TOKEN="${answers.telegramToken.trim()}"\n`;
    if (answers.braveKey.trim())
        envContent += `BRAVE_API_KEY="${answers.braveKey.trim()}"\n`;
